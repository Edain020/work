...
ID3D11RasterizerState *g_pRasterizerState[2];
...
HRESULT InitDX(HWND hWnd, UINT width, UINT height, bool fullscreen)
{
	...
	// ウィンドウモードとフルスクリーンモードの切り替えを可能にする設定
	...
	UINT numDriverTypes = ARRAYSIZE(driverTypes);	// ドライバの種類の数
	...
	// テクスチャのフォーマット
	// 本来の画像フォーマとはRGBA各8bitのデータになる。
	// 深度を8bitで表そうすると、256段階でしか奥行きを表現できない
	// より詳細に奥行きを表現するために、深度バッファでは24bit,
	// または32bit使用して奥行きを画像フォーマットがある
	// 注意点として、カメラの映す奥行きが遠すぎると、
	// 微妙な前後関係がデータの丸め誤差として表現できなくなる
	// 例えば1m間隔の前後関係があるとき、カメラの奥行きが10mだと
	// 奥行きの差は0.1になる。奥行きが1000mだと差が0.001となり、
	// 丸め誤差で差がないものとして扱われることがある
	// 差がないと二つのオブジェクトが同一の場所にあるものと判断され、
	// お互いに画面に映ろうとして、結果その一部がチラつくことがある
	// この現象をZファイティングと呼ぶ
	depthTexDesc.Format = DXGI_FORMAT_D24_UNORM_S8_UINT;
	hr = g_pDevice->CreateTexture2D(&depthTexDesc, NULL, &pDepthTexture);
	if (FAILED(hr)) { return E_FAIL; }

	// 作成したテクスチャを用いて深度バッファの作成
	...
	// 設定
	// 2Dの描画を行うときはZバッファを設定しない(無効にする)
	// 3Dの描画ではZバッファを設定する(有効にする)
	...
	// ラスタライザ
	CD3D11_RASTERIZER_DESC rrDesc = {};
	ZeroMemory(&rrDesc, sizeof(rrDesc));
	// カリング対象の面を設定
	...
	hr = g_pDevice->CreateRasterizerState(
		&rrDesc, &g_pRasterizerState[0]);
	if (FAILED(hr)) { return hr; }
	// カリング無効
	rrDesc.CullMode = D3D11_CULL_NONE;
	hr = g_pDevice->CreateRasterizerState(
		&rrDesc, &g_pRasterizerState[1]);
	if (FAILED(hr)) { return hr; }
...
void UninitDX()
{
	SAFE_RELEASE(g_pRasterizerState[0]);
	SAFE_RELEASE(g_pRasterizerState[1]);
	for (int i = 0; i < BLEND_MAX; ++i)
	{
...
void BeginDrawDX()
{
	...
	g_pContext->ClearDepthStencilView(
		...
		// 深度値を初期化したときの値(カメラが映せる範囲は
		// 最終的に0~1の範囲に丸められる。そのため、1が一番奥になる)
		1.0f, 0);
}
void EndDrawDX()
{
...
{
	g_pContext->RSSetState(g_pRasterizerState[enable ? 0 : 1]);
}
